...
export const deleteTopicAndPosts = async (topicId: string): Promise<{ topic: Topic | null; deletedPostsCount: number }> => {
  try {
...
export const deleteTopicAndUpdatePosts = async (topicId: string): Promise<{ topic: Topic | null; updatedPostsCount: number }> => {
  try {
    const topic = await TopicModel.findByIdAndDelete(topicId);
